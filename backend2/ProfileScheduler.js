...
class ProfileScheduler {
  ...
  async ensureProfilesExist() {
    ...
    try {
      for (const username of this.profiles) {
        const checkQuery = 'SELECT username FROM ticktok_users WHERE username = ?';
        const [rows] = await connection.execute(checkQuery, [username]);
        
        if (rows.length === 0) {
          console.log(`📝 Profile ${username} not in database, will be added on first scrape`);
        }
      }
  ...
  async scheduleProfiles() {
    try {
      ...
      for (const profile of dueProfiles) {
        ...
        const priority = profile.minutes_since_last_scrape === 'never' ? 0 : 
                        Math.max(0, 300 - profile.minutes_since_last_scrape); // Higher priority for older scrapes

        const success = await this.queueManager.addJob(profile.username, priority);
        if (success) {
          addedCount++;
        }
      }

      console.log(`✅ Added ${addedCount}/${dueProfiles.length} profiles to scraping queue`);
      return addedCount;
    } catch (error) {
  ...
  async start() {
    ...
    console.log(`   Scrape interval: ${this.scrapeInterval / 60000} minutes per profile`);
    ...
    this.schedulerInterval = setInterval(async () => {
      ...
      try {
        ...
        console.log(`📊 Queue stats: ${stats.pending} pending, ${stats.processing} processing, ${stats.retry} retry`);
