...
const CHROME_VERSIONS = [
  { version: '139', brand: '"Not;A=Brand";v="99", "Google Chrome";v="139", "Chromium";v="139"' },
  { version: '138', brand: '"Not;A=Brand";v="24", "Chromium";v="138", "Google Chrome";v="138"' },
  { version: '137', brand: '"Not;A=Brand";v="99", "Google Chrome";v="137", "Chromium";v="137"' }
];
  ...
  try {
    ...
    cookieLines.forEach(cookieLine => {
      ...
      if (name && value) {
        ...
        if (['msToken', 'ttwid', 'tt_csrf_token', 'tt_chain_token', 'tt_webid_v2'].includes(cookieName)) {
          tokens[cookieName] = cookieValue;
          console.log(`✅ Found ${cookieName}: ${cookieValue.substring(0, 20)}...`);
        }
      }
    });
  } catch (e) {
...
async function extractTikTokTokens(username, retries = 3) {
  ...
  try {
    ...
    page.on('request', (req) => {
      ...
      if (url.includes('/api/post/item_list/') && !tokensCaptured && cookieTokens.msToken) {
        ...
        try {
          ...
          capturedTokens = {
            msToken: cookieTokens.msToken, // Use msToken from cookies instead of URL
          ...
          tokensCaptured = true;
          console.log('🎉 Tokens successfully captured using msToken from cookies');
    ...
    page.on('response', async (response) => {
      ...
      if (url.includes(`/@${username}`) && response.status() === 200) {
        ...
        try {
          ...
          if (setCookieHeaders) {
            console.log(`🔍 Found set-cookie header with ${setCookieHeaders.split('\n').length} cookies`);
            ...
            console.log('✅ Total extracted cookie tokens:', Object.keys(cookieTokens));
          } else {
    ...
    try {
      await page.goto(url, {
        waitUntil: randomChoice(['networkidle0', 'networkidle2', 'domcontentloaded']),
    ...
    if (!capturedTokens.msToken || !capturedTokens.secUid) {
      throw new Error(`Missing essential tokens - msToken: ${!!capturedTokens.msToken}, secUid: ${!!capturedTokens.secUid}`);
    }
    ...
    if (retries > 0) {
      ...
      const backoffDelay = Math.min(30000, (4 - retries) * 5000 + randomInt(1000, 5000));
...
class TokenExtractorSession {
  ...
  async getTokens(username, forceRefresh = false) {
    ...
    if (!forceRefresh && cached && now - cached.timestamp < 300000) { // 5 min cache
    ...
    try {
      ...
      console.log(`🚀 Extracting fresh tokens for @${username} (request #${this.requestCount})...`);
